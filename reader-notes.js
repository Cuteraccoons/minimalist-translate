...
(() => {
  ...
  function attach(root,{title,url}){
    ...
    const edit=item=>{
      ...
      editor.querySelector('[data-save]').addEventListener('click',async()=>{const previous=item.note;item.note=editor.querySelector('textarea').value;try{await persist();paint(item);render();closeEditor();}catch{item.note=previous;}});
    };
