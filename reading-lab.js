(() => {
  const sites=[
    ['长篇书籍','《原始文化》卷一','https://www.gutenberg.org/files/70458/70458-h/70458-h.htm','你补充的重点案例：完整章节、页码、长段落和脚注。'],
  ...
  function render(){
    ...
    for(const group of [...new Set(sites.map(site=>site[0]))]){
      ...
      const section=el('section'),grid=el('div');grid.className='lab-grid';section.append(el('h2',group),grid);host.append(section);
      matching.forEach(([category,title,url,hint])=>{
        ...
        const note=el('textarea');note.placeholder='例如：第 3 章缺段、脚注没有跳转…';note.setAttribute('aria-label',`${title}的检查备注`);note.value=record.note||'';
        ...
        card.append(el('small',new URL(url).hostname),el('h3',title),el('p',hint),link,label,note);grid.append(card);
      });
    }updateCount();
  }
