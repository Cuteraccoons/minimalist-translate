...
(() => {
  ...
  async function compose({image, rect, viewport, title, url, color, showTitle=true, showLink=true, showQR=true, signature='', layout='card'}) {
    ...
    let width=Math.max(420,rect.width+64);const padding=layout==='compact'?18:32;const header=layout==='editorial'&&showTitle?90:0;
    ...
    const footer=hasFooter ? Math.max(showQR?qrSize+56:0,(showTitle&&layout!=='editorial'?66:0)+(signature.trim()?44:0)+(showLink?24:0)+40) : 0;
    const canvas=document.createElement('canvas');canvas.width=Math.round(width*density);canvas.height=Math.round((rect.height+padding*2+footer+header)*density);
    ...
    ctx.save();ctx.beginPath();ctx.roundRect(x,padding+header,rect.width,rect.height,layout==='editorial'?3:12);ctx.clip();
    ctx.drawImage(shot,rect.x*density,rect.y*(shot.naturalHeight/viewport.height),rect.width*density,rect.height*(shot.naturalHeight/viewport.height),x,padding+header,rect.width,rect.height);ctx.restore();
    if(!hasFooter)return canvas;
    const qx=width-padding-qrSize,qy=rect.height+padding+header+28;
    ...
    if(showTitle){if(layout==='editorial')drawLines(title,padding+24,20,2,width-padding*2);else y=drawLines(title,y,20,2)+8;}
