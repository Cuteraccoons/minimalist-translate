...
新增百科阅读适配、表格色阶保留与阅读设置改进。1.0.3 安装包仍单独保留。

### 1.0.3 更新

新增分步使用指南与阅读测试台，修复本地 OCR 启动和模型缓存，并改善长篇电子书提取。加载源码或本地候选版后，可在「设置 → 项目与赞赏」打开指南或测试台。检查记录保存在本机，可导出反馈。商店与 GitHub Release 的版本可能晚于源码。
...
- 高亮和笔记按文章保存在本机，收藏页支持按文章折叠查看，以及按时间、来源网站筛选后导出。
