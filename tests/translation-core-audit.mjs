...
if (!content.includes('function ensureReaderStylesheet') || !content.includes('collectReaderMediaEntries') || !css.includes('.reader-context-panel')) fail('lazy three-column reader workspace missing');
if (!content.includes('readerTableHtml') || !content.includes('data-reader-nav-tab="media"') || !css.includes('.reader-infobox')) fail('reader media index or Wikipedia table treatment missing');
if (!content.includes('readerTableCellPairHtml') || !content.includes('reader-semantic-table') || !css.includes('.reader-table-scroll')) fail('semantic reader table preservation missing');
if (!content.includes('<colgroup>') || !content.includes('data-reader-align=') || !css.includes('[data-reader-align="center"]')) fail('reader table proportions or source alignment preservation missing');
if (!content.includes('readerDetailsHtml') || !content.includes('readerEmbeddedMediaHtml') || !css.includes('.reader-details-block')) fail('complex reader modules missing');
if (!content.includes('readerToolsCollapsed') || !background.includes('readerToolsCollapsed') || !css.includes('.reader-tools-collapsed')) fail('reader side panels are not independently collapsible');
if (!content.includes('readerToolsWidth') || !background.includes('readerToolsWidth') || !css.includes('.reader-context-resizer')) fail('reader tool rail resizing is not persisted');
if (!content.includes('reader-nav-search-input') || !css.includes('.reader-search-hit')) fail('reader in-document search missing');
if (!content.includes('reader-settings-inline') || !css.includes('.reader-settings-drawer.reader-settings-inline.open')) fail('reader detailed settings do not unfold inline');
if (!content.includes('readerWikipediaFlow') || !background.includes('readerWikipediaFlow') || !css.includes('.reader-wikipedia-flow .reader-infobox')) fail('optional Wikipedia fact-card flow missing');
if (!content.includes('readerWikipediaMagazine') || !background.includes('readerWikipediaMagazine') || !css.includes('.reader-wikipedia-magazine .reader-content')) fail('optional Wikipedia magazine layout missing');
if (!css.includes('column-count:2!important') || !css.includes('object-fit:contain!important') || !css.includes('height:auto!important')) fail('proportional reader media wall missing');
if (/\.reader-infobox[^\{]*\{[^}]*grid-template-columns\s*:\s*repeat\(2,minmax\(0,1fr\)\)/.test(css)) fail('reader infobox still forces unrelated cards into two columns');
...
if (!content.includes('new Set(["card", "flat", "safari", "forum"])') || (content.match(/data-reader-surface=/g) || []).length !== 4) fail('reader page surface choices missing');
...
if (content.includes('speechSynthesis.cancel(')) fail('reader speech must not cancel speech owned by another extension');
if (!content.includes('window.addEventListener("pointermove", move, true)') || !css.includes('.reader-tools-resizing')) fail('reader tool rail drag tracking is still fragile');
if (!css.includes('--reader-ui-font:"Source Han Sans SC"') || !content.includes('case "smiley-sans"') || !background.includes('readerFontDefaultV2') || !background.includes('translationFontDefaultV2')) fail('reader UI/default font migration is missing');
...
if (!layoutMatrix.includes('const TOTAL_CASES = 180') || !layoutMatrix.includes('dataset.surfaceTone') || !layoutMatrix.includes('window.runBilingualLayoutAudit') || !layoutMatrix.includes('window.runReplacementLayoutAudit')) fail('180-case layout matrix fixture missing');
