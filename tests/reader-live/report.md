...
- 修复长条图被比例过滤、旗帜被文件名判作行内图标；配图不继承宿主页裁切与变形。
- NASA Moon/Sun 使用完整文章容器，过滤 Contents 侧栏；轮播文字展开为连续阅读。
- 少数派提取正文外的文章封面；引用使用小引号与缩进。
- 博客园把带行号的 HTML 代码表还原成逐行代码，避免重复表格；代码固定高对比度并保留缩进。
- 京都地区色块列表转为正文条目；真实气候/路线表仍保留，复杂图表未承诺复刻原站交互。
- 大纲取消三行截断；正文标题去掉装饰线，注音段落加大行距。
- Reddit 回复提供独立折叠箭头、头像与嵌套留白；真实 Reddit 访问仍受限，交互使用自有论坛页面验证。
- 浏览器结构复测：柏林、NASA 月球/太阳、京都、少数派、博客园、阮一峰；无截图、无真实翻译调用。
- 百度百科人类学页面本轮自动浏览器未取得正文，已添加本机检查入口，不计为验证通过。
