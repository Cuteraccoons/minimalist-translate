async page => {
  ...
  return await page.evaluate(async () => {
    ...
    const ctx=canvas.getContext("2d");ctx.fillStyle="#f7f2e8";ctx.fillRect(0,0,720,220);
    ctx.fillStyle="#17191c";ctx.font="700 58px Arial";ctx.fillText("HELLO WORLD",72,128);
    ...
    return await new Promise((resolve,reject) => {
      ...
      const onMessage=event=>{
        ...
        if(msg.type==="result"){
          ...
          cleanup();
          if(!/HELLO/i.test(result.text)||result.lines<1)reject(new Error(`OCR 结果不完整：${JSON.stringify(result)}`));
          else resolve(result);
        }
      };
